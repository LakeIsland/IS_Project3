...
int main(int argc, char** argv){
    ...
    while(running){
		...
		traj next_goal = path_RRT.at(look_ahead_idx);
			...
			float speed = getLinearlyInterpolatedValue(0, CAR_TARGET_MAX_SPEED, 0.2, CAR_TARGET_MIN_SPEED, myabs(control));

			printf("speed : %.3f, alpha, %.3f\n", speed, control);
...
void generate_path_RRT()
{
	...
	for(int i = 1; i<goal_number; i++)
	{	
		point x_goal = waypoints[i];

		rrtTree* _rrtTree = new rrtTree(x_init, x_goal, map, map_origin_x, map_origin_y, res, margin);
		_rrtTree -> generateRRT(world_x_max, world_x_min, world_y_max, world_y_min, K, MaxStep);
		printf("%d path created\n", i);
